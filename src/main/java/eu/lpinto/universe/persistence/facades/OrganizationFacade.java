...
public class OrganizationFacade extends AbstractFacade<Organization> {
    ...
    private List<Organization> findByUser(final Long userID) {
        return getEntityManager()
                .createQuery("SELECT o FROM Organization o WHERE o.id IN (SELECT w.organization.id FROM Worker w WHERE w.employee.user.id = :userID AND w.enable = true) AND o.enable = true", Organization.class)
